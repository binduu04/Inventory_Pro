...
#         # 🔥 1. Check if PaymentIntent already exists
#         # existing = (
...
def create_payment_intent():
    try:
        ...
        # ✅ SAFE: always returns .data (even empty)
        existing = (
            supabase.table("payment_intents")
            .select("*")
            .eq("user_id", user_id)
        ...
        # If PaymentIntent already exists for this user+key → reuse it
        if existing.data and len(existing.data) > 0:
            return jsonify({
                "clientSecret": existing.data[0]["client_secret"]
            }), 200
...
# CONFIRM PAYMENT ROUTE (unchanged except PaymentIntent verify)
@cart_bp.route('/confirm-payment', methods=['POST'])
@verify_token
@require_role(['customer'])
def confirm_payment():
    try:
        ...
        # (Everything else remains SAME as your previous logic...)
        # stock validation
        # sale insertion
        # sale_items insertion
        # stock update
